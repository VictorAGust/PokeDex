...
const Pokemon = ({ pokemon }) => {
    ...
    const renderTypes = () => (
        pokemon.types.map(type => (
            <li key={type.slot} className='px-6 py-1 bg-slate-700 rounded text-black font-extrabold uppercase'>
                {type.type.name}
            </li>
        ))
    )
    ...
    const renderStats = () => (
        pokemon.stats.map((stat, index) => (
            <div key={index} className='bg-slate-700 my-2 rounded p-1 mr-36'>
                <div className='bg-slate-900 rounded px-4'style={{width: `${stat.base_stat}%`}}>
                    {stat.base_stat}
                </div>
            </div>
        ))
